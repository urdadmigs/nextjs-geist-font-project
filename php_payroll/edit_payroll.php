...
try {
    ...
    $_SESSION['error'] = "Error fetching payroll data.";
    header('Location: payroll.php');
    exit();
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$employee_id || empty($pay_date)) {
        ...
        try {
            // Check if a payroll record already exists for this employee and date (excluding current record)
            $stmt = $pdo->prepare("
                SELECT COUNT(*) FROM payroll 
                WHERE employee_id = ? AND pay_date = ? AND id != ?
            ");
            $stmt->execute([$employee_id, $pay_date, $id]);
            if ($stmt->fetchColumn() > 0) {
                ...
                $stmt = $pdo->prepare("
                    UPDATE payroll 
                    SET employee_id = ?, pay_date = ?, gross_salary = ?, 
                        deductions = ?, bonuses = ?, net_salary = ?
                    WHERE id = ?
                ");
                $stmt->execute([
                    $employee_id, $pay_date, $gross_salary, 
                    $deductions, $bonuses, $net_salary, $id
                ]);

                $_SESSION['success'] = "Payroll record updated successfully.";
                header('Location: payroll.php');
                exit();
            }
        } catch (PDOException $e) {
            error_log("Update Payroll Error: " . $e->getMessage());
            $error = 'Error updating payroll record. Please try again.';
        }
    }
}
            ...
            <div class="row">
                    ...
                    <select class="form-select" id="employee_id" name="employee_id" required>
                        ...
                        <?php foreach ($employees as $employee): ?>
                            <option value="<?php echo $employee['id']; ?>" 
                                    ...
                                    <?php echo $payroll['employee_id'] == $employee['id'] ? 'selected' : ''; ?>>
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <input type="date" class="form-control" id="pay_date" name="pay_date" 
                           value="<?php echo htmlspecialchars($payroll['pay_date']); ?>" required>
                ...
                <div class="col-md-4 mb-3">
                    ...
                    <div class="input-group">
                        ...
                        <input type="number" class="form-control" id="gross_salary" name="gross_salary" 
                               step="0.01" min="0" 
                               value="<?php echo htmlspecialchars($payroll['gross_salary']); ?>" required>
                ...
                <div class="col-md-4 mb-3">
                    ...
                    <div class="input-group">
                        ...
                        <input type="number" class="form-control" id="deductions" name="deductions" 
                               step="0.01" min="0" 
                               value="<?php echo htmlspecialchars($payroll['deductions']); ?>" required>
                ...
                <div class="col-md-4 mb-3">
                    ...
                    <div class="input-group">
                        ...
                        <input type="number" class="form-control" id="bonuses" name="bonuses" 
                               step="0.01" min="0" 
                               value="<?php echo htmlspecialchars($payroll['bonuses']); ?>" required>
                ...
                <div class="col-md-4">
                    ...
                    <div class="input-group">
                        ...
                        <input type="text" class="form-control" id="net_salary" readonly 
                               value="<?php echo htmlspecialchars($payroll['net_salary']); ?>">
                    </div>
                </div>
            </div>
